...
var TKAppInventoryGRList = function () {
    var table;
    var datatable;
    var mobileContainer;
    ...
    var initDataTable = function () {
        table = document.querySelector('#kt_inventory_gr_table');
        mobileContainer = document.querySelector('#kt_inventory_gr_mobile_container');
        mobilePagination = document.querySelector('#kt_inventory_gr_mobile_pagination');
        ...
        datatable = $(table).DataTable({
            ...
            autoWidth: false,
            ajax: {
                url: hostUrl + "ajax/inventory/stocks/gr_list",
                type: "POST",
                data: function(d) {
                    d.filters = {
            ...
            columnDefs: [
                ...
                {
                    targets: 2,
                    render: function (data) { return `<a href="${hostUrl}inventory/purchase-requisition/approval/${data}" class="text-primary fw-bold">${data}</a>`; }
                },
                {
                    targets: 5,
        ...
        data.each(function (row, index) {
            const card = `
                <div class="card card-flush border-bottom rounded-0" data-kt-element="item">
                    <div class="card-header min-h-auto py-4 px-5 collapsible cursor-pointer collapsed" data-bs-toggle="collapse" data-bs-target="#kt_gr_list_item_${index}">
                        <div class="card-title m-0">
                            <div class="d-flex flex-column">
                                <span class="text-gray-800 fw-bold fs-6">${row.id}</span>
                                <span class="text-muted fs-8">${row.date} • ${row.supplier}</span>
                            </div>
                        </div>
                    ...
                    <div id="kt_gr_list_item_${index}" class="collapse">
                        <div class="card-body p-5 pt-0">
                            <div class="separator separator-dashed my-3"></div>
                            <div class="d-flex flex-stack fs-7 mb-1"><span class="text-gray-500">No. PR:</span><span class="text-primary fw-bold">${row.pr_id}</span></div>
                            ...
                            <div class="d-flex justify-content-end">
                                <button class="btn btn-sm btn-light-primary btn-flex btn-center px-6" data-kt-menu-trigger="click" data-kt-menu-placement="bottom-end">
                                    Aksi GR <i class="ki-duotone ki-down fs-5 ms-2"></i>
                                </button>
                                <div class="menu menu-sub menu-sub-dropdown menu-column menu-rounded menu-gray-600 menu-state-bg-light-primary fw-semibold fs-7 w-150px py-4" data-kt-menu="true">
        ...
        let html = `
            <div class="d-flex flex-stack flex-wrap gap-3 fs-7">
                ...
                <div class="d-flex gap-2">
                    <button class="btn btn-sm btn-light ${info.page === 0 ? 'disabled' : ''}" onclick="TKAppInventoryGRList.setPage('prev')">Sebelumnya</button>
                    <button class="btn btn-sm btn-light ${info.page === info.pages - 1 ? 'disabled' : ''}" onclick="TKAppInventoryGRList.setPage('next')">Selanjutnya</button>
                </div>
            </div>`;
    ...
    var handleSearchDatatable = () => {
        const filterSearch = document.querySelector('[data-kt-inventory-gr-filter="search"]');
    ...
    var updateActiveFilters = function() {
        ...
        $('#clear-all-filters').on('click', function(e) { e.preventDefault(); flatpickr.clear(); $('#filter_supplier').val(null).trigger('change'); });
    };
...
KTUtil.onDOMContentLoaded(function () { TKAppInventoryGRList.init(); });
